...
describe("Token structure contract", () => {
  it("has required top-level structure", async () => {
    ...
    // Must have ivy-framework at top level
  ...
  it("theme colors reference source colors", async () => {
    ...
    const lightColors = tokens["ivy-framework"].theme.light.color;
    const darkColors = tokens["ivy-framework"].theme.dark.color;
  ...
  it("token references use correct format", async () => {
    ...
    const checkReferences = (colors: any) => {
      for (const [key, token] of Object.entries(colors)) {
        const value = (token as any).value;
        // If it's a reference, it should match {ivy-framework.source.color.xxx} or {ivy-web.source.color.xxx} format
        if (value.startsWith("{") && value.endsWith("}")) {
          expect(value).toMatch(/^\{(ivy-framework|ivy-web)\.source\.color\.[\w-]+\}$/);
        }
      }
    };
