...
describe("Output Values Contract", () => {
  describe("C# NuGet Package - Raw Color Values", () => {
        ...
        theme: {
          light: {
            color: {
              ...
              destructive: { value: "{ivy-framework.source.color.destructive}", type: "color" },
            },
          },
        },
      };
      ...
      const darkTheme = {
        theme: {
          dark: {
            color: {
              primary: { value: "{ivy-framework.source.color.primary}", type: "color" },
              background: { value: "{ivy-framework.source.color.black}", type: "color" },
              foreground: { value: "{ivy-framework.source.color.muted-light}", type: "color" },
            },
          },
        },
      };
    ...
    it("ensures all token values are valid color formats", async () => {
      ...
      for (const match of matches) {
        ...
        expect(value).not.toMatch(/^\{(ivy-framework|ivy-web)\.source\./);
    ...
    it("verifies actual token file generates outputs with raw values", async () => {
      try {
        ...
        if (primaryMatch) {
          ...
          expect(primaryValue).not.toMatch(/^\{(ivy-framework|ivy-web)\.source\./);
        }
      } catch (error) {
  ...
  describe("Contract: No References in Final Outputs", () => {
    it("ensures C# code never contains unresolved references", () => {
      ...
      expect(codeWithSource).not.toMatch(/\{(ivy-framework|ivy-web)\.source\.color\./);
    ...
    it("ensures all theme token classes resolve references", () => {
      ...
      expect(lightCode).not.toMatch(/\{(ivy-framework|ivy-web)\.source\.color\./);
      expect(darkCode).not.toMatch(/\{(ivy-framework|ivy-web)\.source\.color\./);
    });
  });
});
